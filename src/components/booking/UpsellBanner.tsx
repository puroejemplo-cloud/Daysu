...
export default function UpsellBanner({ selectedAssetIds, onAdd, addedIds }: Props) {
  ...
  return (
    <div className="space-y-2 mb-4">
      <p className="text-xs font-black uppercase tracking-widest" style={{ color: "#FF3DA8" }}>✦ Recomendaciones especiales</p>
      {visible.map((s) => (
        <div key={s.ruleId} className="flex items-center gap-3 rounded-xl p-3 border"
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mt-0.5">
              ...
              <span className="text-sm font-black" style={{ color: "#FF3DA8" }}>
                {s.discountPercent > 0 ? `$${s.discountedPrice.toFixed(2)}` : "Add-on disponible"}
              </span>
              {s.discountPercent > 0 && (
                <span className="text-xs font-black px-1.5 py-0.5 rounded"
                  style={{ background: "rgba(212,175,55,.15)", color: "#FF3DA8" }}>
                  -{s.discountPercent}%
                </span>
              )}
            </div>
